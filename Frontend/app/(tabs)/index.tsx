...
function ReceiverDetails({ receiver, onGoBack, onDonationSubmit }: { receiver: any; onGoBack: () => void; onDonationSubmit: (donationDetails: any) => void }) {
  ...
  const handleSubmit = () => {
    const donationDetails = {
      donorName: 'John Doe', // Replace with actual donor name
      ngoName: receiver.ngoName,
      foodItems,
      quantity,
    };
    onDonationSubmit(donationDetails);
    onGoBack();
  };
...
// Activity Component
function Activity({ onGoBack, activityLog }: { onGoBack: () => void; activityLog: any[] }) {
  const { isDarkMode } = React.useContext(ThemeContext);
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, isDarkMode && styles.darkCard]}>
            {activityLog.map((log, index) => (
              <Text key={index} style={[styles.cardText, isDarkMode && styles.darkText]}>
                {log.donorName} donated {log.quantity} {log.foodItems} to {log.ngoName}
              </Text>
            ))}
          </View>
        </View>
...
export default function KyndKartApp() {
  ...
  const handleDonationSubmit = (donationDetails: any) => {
    setActivityLog([...activityLog, donationDetails]);
  };
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'activity':
        return <Activity onGoBack={() => setCurrentScreen('home')} activityLog={activityLog} />;
      case 'account':
